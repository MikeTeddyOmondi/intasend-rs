...
Recurring billing: plans, customers, and the subscriptions that join them.

:::caution[Not yet verified against a live endpoint]
The Subscriptions module shipped in `0.4.0` and its request and response shapes
were written from the published OpenAPI definition. Every endpoint path has been
checked against that spec, but **no call has been made against the sandbox**.

Treat it as usable and unproven. If you exercise it, the two things worth
watching are whether `amount` is accepted as a decimal string in practice, and
whether `frequency_unit` and `billing_cycles` can genuinely be omitted — the
spec says they are optional and the crate currently requires them.
:::
...
| Area | Methods |
|---|---|
| Plans | `create_plan`, `list_plans`, `plan_details`, `update_plan` |
| Subscriptions | `create`, `list`, `details`, `update`, `unsubscribe` |
| Customers | `create_customers`, `list_customers`, `customer_details`, `update_customer` |
| Transactions | `transactions` |

## Currencies

Subscriptions accept a different set from the collection endpoints:

**`KES`, `GHS`, `NGN`, `UGX`, `TZS`, `XAF`, `XOF`**

`USD`, `EUR` and `GBP` are valid elsewhere in the API and are **rejected** here.
All of them live on the same `Currency` enum, so the compiler will not stop you
passing one the endpoint refuses — the API is the authority.

## Example

A worked example threading `plan_id` and `customer_id` through every method
lives in [`examples/src/bin/subscriptions.rs`](https://github.com/MikeTeddyOmondi/intasend-rs/blob/master/examples/src/bin/subscriptions.rs).

## Worked example

The whole of `examples/src/bin/subscriptions.rs`, threading each call's output into the
...
```sh
INTASEND_PUBLIC_KEY=... INTASEND_SECRET_KEY=... cargo run --bin subscriptions
```

```rust title="examples/src/bin/subscriptions.rs"
#![allow(unused)]
...
[View on GitHub](https://github.com/MikeTeddyOmondi/intasend-rs/blob/master/examples/src/bin/subscriptions.rs)
