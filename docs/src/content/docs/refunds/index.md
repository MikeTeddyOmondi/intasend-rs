...
The whole of `examples/src/bin/refunds.rs`, threading each call's output into the
...
```sh
INTASEND_PUBLIC_KEY=... INTASEND_SECRET_KEY=... cargo run --bin refunds
```

```rust title="examples/src/bin/refunds.rs"
#![allow(unused)]
...
[View on GitHub](https://github.com/MikeTeddyOmondi/intasend-rs/blob/master/examples/src/bin/refunds.rs)
