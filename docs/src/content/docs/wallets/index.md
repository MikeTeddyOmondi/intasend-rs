...
The whole of `examples/src/bin/wallets.rs`, threading each call's output into the
...
```sh
INTASEND_PUBLIC_KEY=... INTASEND_SECRET_KEY=... cargo run --bin wallets
```

```rust title="examples/src/bin/wallets.rs"
#![allow(unused)]
...
[View on GitHub](https://github.com/MikeTeddyOmondi/intasend-rs/blob/master/examples/src/bin/wallets.rs)
